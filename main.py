...
async def on_voice_state_update(member, before, after):
    ...
    if len(voice.channel.members) > 1:
        if member == bot.user:
            await send_welcome_message(voice.channel, f" <:pinkdotlkn:1196832522319962193> Hello <@{member.id}> <a:3Kellylkng:1192105215474798632> bạn vừa rơi vào phòng <#{after.channel.id}> chơi vui bạn nhé <a:3yhuggingtoy13:1187592428748820552>.")
            return
    ...
    if before.channel != after.channel:
        if after.channel is not None:
            if member == bot.user:
                return
            await send_welcome_message(after.channel, f"<:pinkdotlkn:1196832522319962193> Hello <@{member.id}> <a:3Kellylkng:1192105215474798632> bạn vừa rơi vào phòng <#{after.channel.id}> chơi vui bạn nhé <a:3yhuggingtoy13:1187592428748820552>.")
            await welcome_message(after.channel)
        if before.channel is not None:
            if member == bot.user:
                return
            await send_welcome_message(before.channel, f" <:pinkdotlkn:1196832522319962193> Bái bai <@{member.id}> <a:0echxutxitlkn:1200476710328610916> lần sau lại ghé nhó <a:aclick:1194095144522235964> .")
